...
export const addNextOccurence = (editor: Editor) => {
	...
	if (wordRange !== null) {
		...
		if (nextOccurenceIndex !== -1) {
			...
			const nextOccurenceIndex2 = doc.indexOf(
				word,
				nextOccurenceIndex + word.length
			);
			nextOccurenceIndex = nextOccurenceIndex2 !== -1 ? nextOccurenceIndex2 : nextOccurenceIndex;
		}
	}
};
